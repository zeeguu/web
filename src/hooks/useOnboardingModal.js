...
export default function useOnboardingModal(api, messageId) {
  ...
  // Fail-safe default: assume already-shown until the backend tells us otherwise.
  // This prevents the modal from flashing during the initial render before the
  ...
  useEffect(() => {
    ...
    api.hasSeenOnboardingMessage(messageId)
      .then((shown) => {
        // Skip the state update if the effect was torn down before we resolved.
        // Otherwise we'd either warn about setting state on an unmounted
        // component, or write the wrong message's status into state.
        if (!cancelled) setAlreadyShown(shown);
      })
      .catch(() => {});

    // Cleanup function. React calls this when the effect re-runs (deps changed)
    // or when the component unmounts. We flip the flag so the .then() callback
