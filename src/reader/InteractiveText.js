...
import { UMRsource } from "./ArticleReader";
...
export default class InteractiveText {
  ...
  translate(word, onSuccess) {
    ...
    this.api.logReaderActivity(UMRsource, this.api.TRANSLATE_TEXT, this.articleInfo.id, word.word);
  }
  ...
  selectAlternative(word, alternative, preferredSource, onSuccess) {
    ...
    this.api.logReaderActivity(UMRsource, this.api.SEND_SUGGESTION,this.articleInfo.id,alternative_info);
  ...
  pronounce(word) {
    ...
    this.api.logReaderActivity(UMRsource, this.api.SPEAK_TEXT, this.articleInfo.id, word.word);
  }
