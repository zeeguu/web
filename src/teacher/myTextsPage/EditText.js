...
export default function EditText() {
  ...
  const [removingCohortID, setRemovingCohortID] = useState(null);
  ...
  // The x on a pill: the same one-click unsharing the My Texts list has, so a
  // teacher who opened the text to fix a typo does not have to go through the
  // dialog to take it out of a class.
  const unshareFromCohort = (cohort) => {
    setRemovingCohortID(cohort.id);
    api.deleteArticleFromCohort(
      articleID,
      cohort.id,
      () => {
        setRemovingCohortID(null);
        setCohortList((current) => current.filter((each) => each.id !== cohort.id));
      },
      () => setRemovingCohortID(null),
    );
  };
      ...
      <s.NarrowColumn>
        ...
        >
          ...
          <ClassPills
            ...
            onRemove={unshareFromCohort}
            removingId={removingCohortID}
            removeDisabled={viewAsStudentAndShareDisabled}
